...
setMethod("c", "countData", function(x, ..., recursive = FALSE) {
  ...
  ndat <- abind::abind(c(list(x@data), lapply(cdl, function(x) x@data)), along = 1)
  nrobs <- list()
  for(ii in seq_along(x@rowObservables))
    nrobs[[ii]] <- abind::abind(c(list(x@rowObservables[[ii]]), lapply(cdl, function(x) x@rowObservables[[ii]])), along = 1)
  ncobs <- list()
  for(ii in seq_along(x@cellObservables))
    ncobs[[ii]] <- abind::abind(c(list(x@cellObservables[[ii]]), lapply(cdl, function(x) x@cellObservables[[ii]])), along = 1)
        ...
        if(type == "FDR") {
          sums <- sapply(ords, function(ord) {
            if(sum(cd@orderings[,gg] == ord) > 0) {
             # tc <- suppressWarnings(baySeq::topCounts(cd[cd@orderings[,gg] == ord,], gg, FDR = 0.05))
...
#' densityFunction method for testClass
#'
#' @docType methods
#' @rdname densityFunction-methods
...
setMethod("[", "countData", function(x, i, j, ..., drop = FALSE) {
  ...
  x@RPKM <- .sliceArray(list(i, j), x@RPKM)
...
#' DGEList method for testClass
#'
#' @docType methods
#' @rdname DGEList-methods
#' @param counts Value
#' @param lib.size Value
#' @param norm.factors Value
#' @param samples Value
#' @param group Value
#' @param genes Value
#' @param remove.zeros Value
...
#' dropEmptyLevels method for testClass
#'
#' @docType methods
#' @rdname dropEmptyLevels-methods
#' @param x Value
...
#' calcNormFactors method for testClass
#'
#' @title MyHappyFunction: The my happy function
#' @docType methods
#' @rdname calcNormFactors-methods
#' @param object Value
#' @param ... Value
#' @export
...
#' calcNormFactors.DGEList method for testClass
#'
#' @docType methods
#' @rdname calcNormFactors
#' @method calcNormFactors DGEList

#' @param object Value
#' @param method Value
#' @param refColumn Value
#' @param logratioTrim Value
#' @param sumTrim Value
#' @param doWeighting Value
#' @param Acutoff Value
#' @param p Value
#' @param ... Value
#' @export
...
#' calcNormFactors.default method for testClass
#'
#' @docType methods
#' @rdname calcNormFactors.default-methods
#' @param object Value
#' @param lib.size Value
#' @param method Value
#' @param refColumn Value
